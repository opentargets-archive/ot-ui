...
const getColumnsAll = schemas => {
  const columns = [
    { id: 'gene', label: 'Gene' },
    { id: 'overallScore', label: 'Overall G2V' },
  ];
...
const getTissueColumns = (schemas, sourceId) => {
  ...
  schemas
    ...
    .tissues.forEach(tissue => {
      columns.push({ id: tissue.id, label: tissue.name });
    });
  return columns;
};
...
class AssociatedGenesTable extends Component {
  state = {
    value: 0,
  };
  ...
  render() {
    ...
    console.log('genesForVariant', genesForVariant);
    console.log('genesForVariantSchema', genesForVariantSchema);
